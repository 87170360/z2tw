...
function MenPaiMainLayer:onInitView(openInfo)
	...
	local name = {[0]="少   林",[1]="武   當",[2]="丐   幫",[3]="峨   眉",[4]="霸   刀",[5]="天   山",[6]="明   教",[7]="五   毒"}
...
function MenPaiMainLayer:onUIButtonClick(sender)
	...
	elseif 5 == idx then
		...
		local cfgItem = cp.getManager("ConfigManager").getItemByKey("Store",1) --門派商店
		if cfgItem then
		...
		self:openMenPaiShop()
	elseif 6 == idx then
		cp.getManager("ViewManager").gameTip("功能正在開發中，盡情期待！")
	end
end
