...
local ShopItem = class("ShopItem",BNode)

function ShopItem:create(goodsInfo)
	local node = ShopItem.new(goodsInfo)
	return node
end

function ShopItem:initListEvent()
	self.listListeners = {
		--購買商品返回
...
function ShopItem:onInitView(goodsInfo)
    self.goodsInfo = goodsInfo
	self.rootView = cc.CSLoader:createNode("uicsb/uicsb_shop/uicsb_shop_item.csb") 
...
function ShopItem:onEnterScene()
    
end

function ShopItem:onExitScene()
   
end

function ShopItem:resetBG(needBG)
	if needBG then
...
function ShopItem:resetInfo(goodsInfo)
	self.goodsInfo = goodsInfo
